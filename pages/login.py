...
def verificar_login(usuario, senha):
    ...
    cursor.execute("SELECT * FROM usuarios WHERE username = ? AND password = ?", (usuario, senha_hash))
...
def area_autenticacao():
    ...
    with aba_cadastro:
        ...
        with st.form("formulario_cadastro"):
            novo_usuario = st.text_input("Escolha um Usuário", placeholder="Ex: joao_silva", key="cad_user")
            nova_senha = st.text_input("Escolha uma Senha", type="password", placeholder="Mínimo 6 caracteres", key="cad_pass")
            confirmar_senha = st.text_input("Confirme a Senha", type="password", placeholder="Repita a senha", key="cad_pass_conf")
            ...
            if botao_cadastrar:
                ...
                if not novo_usuario or not nova_senha:
                    ...
                    if cadastrar_novo_usuario(novo_usuario.strip(), nova_senha):
                        st.success(f"Usuário '{novo_usuario}' cadastrado com sucesso! Agora você pode mudar para a aba 'Acessar Conta'.")
                    else:
                        st.error(f"O nome de usuário '{novo_usuario}' já está em uso. Escolha outro.")
